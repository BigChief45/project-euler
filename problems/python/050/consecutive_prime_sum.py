...
if __name__ == '__main__':
    top = 1000 # One million
    
    # Get all primes up to one million
    primes = []
    for i in xrange(1, top + 1):
        if MillerRabinPrimalityTest(i):
            primes.append(i)
    
    for prime in primes:
        sys.stdout.write("%s = " % (prime))
        
        # Iterate through all primes below the current prime
        prime_sum = 0
        for k in primes:
            prime_sum = prime_sum + k
            if (prime_sum >= prime) or (k == prime):
                break
            
            sys.stdout.write("%s + "  % (k))
        
        sys.stdout.write(" = %s" % prime_sum)
        if not prime_sum == prime : sys.stdout.write(" [X]") 
